...
#if SRTM_SAI_EDMA_PLAY_HALT_WORKAROUND
            /* In armgcc release, DMA5 driver transfer have issue in some special usecase in IMX95, ues this delay as workaround. */
...
static void SRTM_SaiEdmaAdapter_ReconfigSAI(srtm_sai_adapter_t adapter,
                                            srtm_audio_dir_t dir,
                                            uint8_t format,
                                            uint32_t srate)
{
    srtm_sai_edma_adapter_t handle = (srtm_sai_edma_adapter_t)(void *)adapter;
#if (defined(SRTM_SAI_CONFIG_Rx_Enabled) && SRTM_SAI_CONFIG_Rx_Enabled)
    srtm_sai_edma_runtime_t rtm = dir == SRTM_AudioDirTx ? &handle->txRtm : &handle->rxRtm;
    srtm_sai_edma_config_t *cfg = dir == SRTM_AudioDirTx ? &handle->txConfig : &handle->rxConfig;
#else
    srtm_sai_edma_runtime_t rtm        = &handle->txRtm;
    srtm_sai_edma_config_t *cfg        = &handle->txConfig;
#endif
    uint32_t saiSrcClk;
    ...
    srtm_sai_adapter_t adapter, srtm_audio_dir_t dir, uint8_t index, uint8_t format, uint8_t channels, uint32_t srate)
{
    ...
    SRTM_SaiEdmaAdapter_ReconfigSAI(adapter, dir, format, srate);
