...
void SM_PINCTRL_SetPinMux(sm_pinctrl_t *sm_pinctrl)
{
    ...
    if ((inputRegister & 0xFFFFU) != 0U)
    {
        ...
        configs[numConfigs].value = (inputRegister - SM_PLATFORM_PINCTRL_DAISYREG_OFF) / 4U;
        numConfigs++;
    ...
    if ((muxRegister != 0U) || (inputRegister != 0U))
    {
        ...
        status = SCMI_PinctrlSettingsConfigure(channel, (muxRegister - SM_PLATFORM_PINCTRL_MUXREG_OFF) / 4U, 0U, attributes, configs);
...
void SM_PINCTRL_SetPinCfg(sm_pinctrl_t *sm_pinctrl)
{
    ...
    if (configRegister != 0U)
    {
        int32_t status = SCMI_ERR_SUCCESS;
        uint32_t attributes =
            SCMI_PINCTRL_SET_ATTR_SELECTOR(SCMI_PINCTRL_SEL_PIN) | SCMI_PINCTRL_SET_ATTR_NUM_CONFIGS(1U);
        scmi_pin_config_t configs;

        configs.type  = SCMI_PINCTRL_TYPE_CONFIG;
        configs.value = configValue;

        status = SCMI_PinctrlSettingsConfigure(channel, (configRegister - SM_PLATFORM_PINCTRL_CFGREG_OFF) / 4U, 0U, attributes, &configs);
        /* Find something wrong ASAP(components/scmi/scmi_common.h: scmi error code) */
        while (status != SCMI_ERR_SUCCESS)
	{
	}
