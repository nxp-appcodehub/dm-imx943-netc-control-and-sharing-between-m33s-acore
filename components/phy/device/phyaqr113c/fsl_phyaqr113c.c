...
#define PHY_FIRMWARE_TIMEOUT_COUNT 10000U
...
status_t PHY_AQR113C_Init(phy_handle_t *handle, const phy_config_t *config)
{
    ...
    do
    {
        ...
        counter--;
    } while ((regValue1 == 0x0U) && (counter != 0U));
    ...
    if (config->autoNeg)
    {
        ...
        {
            regValue1 &= (uint16_t)(~(AN_EEE_ADV_100BT_MASK | AN_EEE_ADV_1000BT_MASK | AN_EEE_ADV_10GBT_MASK |
                                      AN_EEE_ADV_100BKX_MASK | AN_EEE_ADV_1000BKX_MASK | AN_EEE_ADV_10GBKX4_MASK |
                                      AN_EEE_ADV_10GBKR_MASK));
        }
...
status_t PHY_AQR113C_SetLinkSpeedDuplex(phy_handle_t *handle, phy_speed_t speed, phy_duplex_t duplex)
{
    ...
    regValue1 &= ~(uint16_t)CTRL1_SPEED_SEL_MASK;
    regValue2 &= ~(uint16_t)CTRL2_TYPE_MASK;
    ...
    regValue1 &= (uint16_t)(~(AN_CTRL1_ENABLE_MASK | AN_CTRL1_RESTART_MASK));
...
status_t PHY_AQR113C_EnableLoopback(phy_handle_t *handle, phy_loop_t mode, phy_speed_t speed, bool enable)
{
    ...
    {
        regValue &= (uint16_t)(~PCS_CTRL1_LOOPBACK_MASK);
    }
